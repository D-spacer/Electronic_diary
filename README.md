...
## Модуль database_manipulation.py

Это дополнительный модуль, позволяющий изменять данные в базе данных. Он содержит три функции:
1. fix_marks - меняет двойки и тройки на пятерки
2. remove_chastisements - удаляет негативные комментарии
3. create_commendation - добавляет случайный положительный комментарий

Все что нужно сделать - указать имя ученика для поиска и в случае с функцией create_commendation - название предмета.
Эти функции можно либо копировать в Django shell, либо имортировать в основной модуль программы manage.py
